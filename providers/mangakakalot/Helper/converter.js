...
async function convertSingle(mangaName, chapterName) {
    ...
    try {
        ...
        await pre(mangaName, chapterName, files, doc);    // calling the pre function so the files are accessible
    }   catch (err) {
        console.error("Error during conversion: ", err);
    }
}

async function pre(mangaName, chapterName, files, doc) {    // a temporary function to execute the pdf commands
